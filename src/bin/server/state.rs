...
impl Store {
    ...
    pub fn add(&mut self, new_vec : dtf::Update, global: &Global) {
        self.size += 1;
        let mut rdr = global.write().unwrap();
    ...
    pub fn flush(&self, global: &Global) -> Option<bool> {
        let folder = global.read().unwrap().settings.dtf_folder.to_owned();
    ...
    /// load size from file
    pub fn load_size_from_file(&mut self, global: &Global) {
    ...
    pub fn clear(&mut self, global: &Global) {
        {
            let mut rdr = global.write().unwrap();
            let vecs = (*rdr).vec_store.get_mut(&self.name).expect("KEY IS NOT IN HASHMAP");
            vecs.clear();
        ...
        self.load_size_from_file(global);
    }
}
...
impl State {
    ...
    pub fn info(&self, global: &Global) -> String {
    ...
    pub fn insert(&mut self, up: dtf::Update, store_name : &str, global: &Global) -> Option<()> {
    ...
    pub fn add(&mut self, up: dtf::Update, global: &Global) {
        let current_store = self.get_current_store();
        current_store.add(up, global);
    }

    pub fn autoflush(&mut self, flush_interval: u32, global: &Global) {
        ...
        if current_store.size % u64::from(flush_interval) == 0 {
            ...
            current_store.flush(global);
            current_store.load_size_from_file(global);
        }
    }

    pub fn create(&mut self, dbname: &str, global: &Arc<RwLock<SharedState>>) {
        {
    ...
    pub fn use_db(&mut self, dbname: &str, global: &Global) -> Option<()> {
        if self.store.contains_key(dbname) {
            self.current_store_name = dbname.to_owned();
            let current_store = self.get_current_store();
            current_store.load(global);
            Some(())
        } else {
            None
        }
    ...
    pub fn get_all_as_json(&mut self, global: &Global) -> String {
        let rdr = global.read().unwrap();
    ...
    pub fn get_n_as_json(&mut self, count: i32, global: &Global) -> Option<String> {
        {
            let current_store = self.get_current_store();
    ...
    pub fn clear(&mut self, global : &Global) {
        self.get_current_store().clear(global);
    }

    pub fn clearall(&mut self, global: &Global) {
    ...
    pub fn flush(&mut self, global: &Global) {
        self.get_current_store().flush(global);
    }
    
    pub fn flushall(&mut self, global: &Global) {
    ...
    pub fn get(&mut self, count : i32, global: &Global) -> Option<Vec<u8>> {
